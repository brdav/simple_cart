...
class DecisionTree:
    ...
    def fit(self, X, t, w=None):
        ...
        if w is None:
            w = np.ones(X.shape[0])
        self.root = self._grow_tree(X, t, w)
        if self.ccp_alpha > 0:
            self._minimal_cost_complexity_pruning(sum(w))
        return self

    def _grow_tree(self, X, t, w, depth=0):
        """
        ...
        impurity_base = self.criterion(t, w)

        if (
            (len(t) < self.min_samples_split)
            ...
            for column_i in select_cols:
                ...
                for split_val in split_vals:
                    t_left = t[X[:, column_i] < split_val]
                    w_left = w[X[:, column_i] < split_val]
                    t_right = t[X[:, column_i] >= split_val]
                    w_right = w[X[:, column_i] >= split_val]
                    # getting the left and right losses
                    impurity_left = self.criterion(t_left, w_left)
                    impurity_right = self.criterion(t_right, w_right)
            ...
            if best_gain <= 0:
                ...
                return TreeNode(
                    None, None, None, None, self.node_value(t, w), impurity_base, sum(w)
                )

            # divide dataset and continue recursion
            X_left = X[X[:, best_column_index] < best_column_threshold]
            t_left = t[X[:, best_column_index] < best_column_threshold]
            w_left = w[X[:, best_column_index] < best_column_threshold]
            X_right = X[X[:, best_column_index] >= best_column_threshold]
            t_right = t[X[:, best_column_index] >= best_column_threshold]
            w_right = w[X[:, best_column_index] >= best_column_threshold]

            left_tree = self._grow_tree(X_left, t_left, w_left, depth=depth + 1)
            right_tree = self._grow_tree(X_right, t_right, w_right, depth=depth + 1)
    ...
    def predict(self, X):
        ...
        y = []
        for x in X:
            y.append(self.root.decide(x))
        return np.asarray(y)

    def node_value(self, t, w):
        """
        ...
        if self.assign_leaf_node == "most_common":
            ...
            w_count = [sum(w[t == k]) for k in unique_classes]
    ...
    def gini(self, t, w):
        ...
        for k in np.unique(t):
            p_tauk = sum(w[t == k]) / sum(w)
            Q_tau -= p_tauk**2
        return Q_tau

    def entropy(self, t, w):
        """
    ...
    def misclassification_rate(self, t, w):
        """
        ...
        w_prob = [sum(w[t == k]) / sum(w) for k in np.unique(t)]
        return sum(sorted(w_prob)[:-1])

    def squared_error(self, t, w):
        """
        ...
        return np.average((t - np.average(t, weights=w)) ** 2, weights=w)
